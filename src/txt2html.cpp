...
int main(int argc, char** argv){
	...
	while(fscanf(fp,"%s\n",doc_id)!=EOF){
		//printf("%s\n",doc_id);
		double sweet,cool,hard,emotion;
		char temp[300] = "";
		fgets(temp, 300, fp);
		...
		//printf("%s\n",temp);
		fscanf(fp,"%lf %lf %lf %lf",&sweet,&cool,&hard,&emotion);
		//<a href = "https://www.ptt.cc/bbs/NTUCourse/M.1263870936.A.127.html" class = "link" id="111"></a>
		
		printf("%s %d %d %d %d %s\n",doc_id,(int)(sweet*100),(int)(cool*100),(int)(hard*100),(int)(emotion*100),temp);
		...
		fprintf(out,"<a href = \"https://www.ptt.cc/bbs/NTUCourse/%s.html\" class = \"link\" id=\"%d+%d+%d+%d+%s\"></a>\n",doc_id,(int)(sweet*100),(int)(cool*100),(int)(hard*100),(int)(emotion*100),temp);
